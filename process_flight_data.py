...
def setup_chromadb():
    ...
    try:
        collection = chroma_client.create_collection(
            name="flight_info", embedding_function=embedding_function
        )
    except ValueError:
...
def main():
    ...
    if collection_exists:
        user_input = input(
            "Collection 'flight_info' already exists. Do you want to:\n"
        ...
        if user_input == "1":
            ...
            flight_data = load_flight_data(Path("data", "flight_data.json"))
    ...
    results = collection.query(
        query_texts=["What are the cancellation charges?"], n_results=2
    )
